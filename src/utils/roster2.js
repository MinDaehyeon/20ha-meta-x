...
// 출석 인정 임계값 (분): 미라클 모닝 15분 이상, 나잇 60분 이상 참여 시 출석
// participation_minutes === null 은 면제·행사(예: 6/3 일정변경, 김도현 영작수업) — 출석 인정
export const MIN_ATTENDANCE_MINUTES = { M: 15, N: 60 };
export const isValidAttendance = (sessionType, minutes) => {
  if (minutes === null || minutes === undefined) return true; // 면제/행사
  const threshold = MIN_ATTENDANCE_MINUTES[sessionType];
