...
### 사전결석 카운팅 한도 적용 (모닝 3·나잇 5)
- 사전결석 기록은 모두 유지하되, **출석 인정은 학생·타입별 회차 빠른 순으로 모닝 3회·나잇 5회까지만**. 초과분은 카운트(숫자)에서만 제외.
- 셀 마커 `신` → **`사전`**(파랑=인정 / 회색=한도초과 미인정)으로 변경.
- 반영: 학생 화면·관리자 전체현황·클래스 통계 RPC(`get_class_attendance_stats`, 윈도우 함수).

---

## v2026.07.10-1 — 2026-07-10

### 신규 (관리자 전체현황 그리드 수동 편집)
- **카페 인증 셀 클릭 → 모달**: 회차 표시 + 제출/미션/충실도 점수(만점 기본) 수정. 글이 없는 칸은 **수동 인증 생성**. 채점표·학생 화면에 그대로 반영.
- **출석 셀 클릭 → 모달**: 출석/결석 선택 + 사전결석 신청 체크(참여시간 무관).
- **사전결석**: 결석이지만 신청한 경우로 기록하되 **출석으로 인정(카운트 O)**. 셀에 파란 `신` 마커로 구분 표시. 한도(모닝 3·나잇 5) 초과분 제외는 다음 단계 예정.
- DB: `attendance_logs.pre_absence`/`pre_absence_at` + `admin_set_attendance`/`admin_create_manual_cert` RPC + 조회/통계 RPC 반영.

---

## v2026.07.06-1 — 2026-07-06

### 버그 수정 (관리자 수동 회차 변경이 학생 화면 미반영)
- **증상:** 관리자가 인증글의 회차를 손으로 바꿔도(예: 한유찬 7회) 학생 "나의 카페 인증 현황"에는 그대로 미제출로 남음.
- **원인:** 학생 화면이 각 인증글의 회차를 작성일(posted_at)로만 자동 계산하고 관리자 수동 회차(session_override)를 무시함. DB·RPC엔 정상 저장돼 있었고 화면 계산 로직만 누락.
- **수정:** `certSessionNums` 헬퍼(session_override 우선 → 없으면 작성일 자동) 도입 후, 회차 완료 set과 회차별 점수 카드 매칭 두 곳에 적용. (dev 프리뷰에서 한유찬 7회 → 100점 반영 확인)
- **범위:** 클라이언트만 수정. 학생 본인·학부모 자녀보기·관리자 학생화면 공통 반영.

---

## v2026.06.18-1 — 2026-06-18

### 버그 수정 (네이버 카페 인증글 링크 첫 로드 빈 화면)
- **증상:** 인증글 제목 링크를 새 탭으로 열면 본문이 안 뜨고 사이드바·제목만 보임. 한 번 새로고침하면 정상.
- **원인:** 저장된 `f-e`(SPA) 주소는 본문을 `ca-fe` iframe(`?fromNext=true`)으로 감싸는데, 콜드 로드 시 이 iframe이 비어버림(네이버 카페 쪽 동작).
- **수정:** 링크를 열 때 `f-e` → `ca-fe`(iframe 안쪽 실제 주소)로 치환. 첫 진입에도 본문이 바로 렌더됨. (Playwright로 콜드 로드 검증)
- **범위:** 표시 시점 치환만 — DB·크롤러 저장 형식은 그대로(중복 비교 로직 보존). 과거 글 링크도 즉시 적용. 관리자 인증글 채점·챌린지 글 목록 2곳.

---

## v2026.06.17-1 — 2026-06-17

### 버그 수정 (출석 "전체 현황" 최신 회차 누락)
- **증상:** 관리자 전체 현황에서 최근 회차(예: 6/8~6/14 미라클나이트)가 비어 보임. "출석이 종종 날아간다"고 느껴지던 현상.
- **원인:** 출석이 1000건을 넘자 PostgREST 기본 1000행 제한에 걸려 최신 회차가 조용히 잘림. (데이터는 DB에 100% 보존 — 표시만 누락)
- **수정:** 전체 현황 로딩을 1000건씩 페이지네이션으로 전부 가져오도록 변경 + `get_attendance_logs` RPC에 `ORDER BY`(session_date, student_id) 추가로 페이지 경계 안정화.
- **영향 범위:** 관리자 전체 현황만 해당. 학생·학부모 화면은 본인 데이터만 조회하므로 영향 없음(정상이었음).
...
### 주요 변경
- **인증글 채점 일괄 처리** — 채점 목록 각 글에 체크박스 추가(헤더에 페이지 전체선택 포함). 여러 글을 체크한 뒤 **‘체크한 글 만점처리(100점)’** 버튼으로 제출 50 / 미션 30 / 충실도 20을 한 번에 저장. 확인 창으로 오작동 방지, 선택 건수 표시·선택 해제 지원.

---

## v2026.06.04-1 — 2026-06-04
**커밋:** [822361d](https://github.com/MinDaehyeon/20ha-meta-x/commit/822361d)

### 주요 변경
- **인증글 점수 입력 기본값** — 미채점 글의 점수 입력창을 열면 만점 기준값(제출 50 / 미션 30 / 충실도 20)이 미리 입력됨. 목록의 빈 점수는 그대로 ‘—’이며, ‘확인’을 눌러야 저장됨(일괄 채움 아님).
- **인증글 영구 제외** — 작성자가 게시판을 착각해 잘못 올린 글(‘[사전미션]김호진/초4/8226’) 삭제. 앞으로 크롤링 시에도 다시 수집되지 않도록 처리.

### 백그라운드 변경
- `cafe_excluded_urls` 블록리스트 테이블 추가. 크롤러가 이 목록의 URL은 건너뜀(삭제해도 재수집 방지).

---

## v2026.06.02-2 — 2026-06-02
**커밋:** [fda9304](https://github.com/MinDaehyeon/20ha-meta-x/commit/fda9304)

### 주요 변경

**출석 시스템 production 배포 (참여 시간 기반)**
- Zoom CSV 업로드로 회차별 참여 시간(분) 기록 — 같은 학생 분리 입퇴장은 합산
- 출석 인정 임계값: 미라클 모닝 15분 이상 / 미라클 나이트 **50분** 이상
- 시간 미달 출석도 DB에 저장하고 학생·관리자 화면에 '참여시간미달'(주황)로 구분 표시
- 학생 8주 일정 셀 + 관리자 전체현황 그리드에 **즉시 뜨는 커스텀 hover tooltip** (날짜·상태·참여 시간·미달 안내)

**운영 예외 — 2026-06-02(나잇)까지 유예**
- 6/2 이전 모든 회차는 참여 기록만 있으면 시간 무관 출석 인정 (기존 미달 61건 전부 인정 복구)
- 6/3 이후 회차부터 나이트 기준 60분 → 50분 적용

### 백그라운드 변경
- DB 마이그레이션 4건 git 기록 반영 (participation_minutes 컬럼, 분 저장 RPC, 조회 RPC, 중복 dedupe) — 공유 Supabase에는 적용 완료
